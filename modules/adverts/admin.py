...
from modules.adverts.forms import AdImageInlineForm
from modules.adverts.models import Ad, Advertiser, Category, AdMedia, Click, Impression
from modules.adverts.utils import get_zones_choices
...
class AdvertiserAdmin(admin.ModelAdmin):
    ...
    def get_changeform_initial_data(self, request):
        ...
        return get_data or {"user": request.user.pk}
...
class AdAdminForm(forms.ModelForm):
    class Meta:
        fields = "__all__"
        widgets = {"zone": forms.Select(choices=get_zones_choices())}
...
class ClickAdmin(admin.ModelAdmin):
    ...
    def download_clicks(self, request, queryset):
        ...
        for click in queryset:
            writer.writerow(
                (
                    unicode(click.ad.title).encode("utf-8"),
                    click.ad.url,
                    click.source_ip,
...
class ImpressionAdmin(admin.ModelAdmin):
    ...
    def download_impressions(self, request, queryset):
        ...
        for impression in queryset:
            writer.writerow(
                (
                    unicode(impression.ad.title).encode("utf-8"),
                    impression.ad.url,
