...
from modules.helpdesk.forms import CUSTOMFIELD_DATE_FORMAT
from modules.helpdesk.query import (
    get_query_class,
    query_to_base64,
    query_from_base64,
)

from modules.helpdesk.user import HelpdeskUser

from modules.helpdesk.decorators import (
    helpdesk_staff_member_required,
    helpdesk_superuser_required,
    is_helpdesk_staff,
)
from modules.helpdesk.forms import (
    TicketForm,
    UserSettingsForm,
    EmailIgnoreForm,
    EditTicketForm,
...
from modules.helpdesk.decorators import superuser_required
from modules.helpdesk.lib import (
...
if helpdesk_settings.HELPDESK_KB_ENABLED:
    from modules.helpdesk.models import KBItem

import modules.helpdesk.views.abstract_views as abstract_views
from modules.helpdesk.views.permissions import MustBeStaffMixin
...
def dashboard(request):
    ...
    unassigned_tickets = active_tickets.filter(
        assigned_to__isnull=True, kbitem__isnull=True, queue__in=user_queues
    )
...
def view_ticket(request, ticket_id):
    ...
    if "subscribe" in request.GET:
        ...
        ticket_cc, show_subscribe = return_ticketccstring_and_show_subscribe(
            request.user, ticket
        )
        if show_subscribe:
...
def subscribe_staff_member_to_ticket(
    ticket, user, email="", can_view=True, can_update=False
):
    """used in view_ticket() and update_ticket()"""
    return subscribe_to_ticket_updates(
        ticket=ticket, user=user, email=email, can_view=can_view, can_update=can_update
    )
...
def update_ticket(request, ticket_id, public=False):
    ...
    f = FollowUp(
        ticket=ticket, date=timezone.now(), comment=comment, time_spent=time_spent
    )
            ...
            try:
                ...
                for attribute, display_name in ticket_attributes:
                    ...
                    if id_for_attribute != chosen_ticket.id:
                        ...
                        # Check if attr is a get_FIELD_display
                        if attribute.startswith("get_") and attribute.endswith(
                            "_display"
                        ):
