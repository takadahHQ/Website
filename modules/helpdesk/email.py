...
from modules.helpdesk import settings
from modules.helpdesk.lib import safe_template_context, process_attachments
from modules.helpdesk.models import Queue, Ticket, TicketCC, FollowUp, IgnoreEmail
...
def process_email(quiet=False):
    for q in Queue.objects.filter(
        ...
        if quiet:
            logger.propagate = (
                False  # do not propagate to root logger that would log to console
            )

        # Log messages to specific file only if the queue has it configured
        if (
            q.logging_type in logging_types
        ) and q.logging_dir:  # if it's enabled and the dir is set
...
def create_object_from_email_message(message, ticket_id, payload, files, logger):
    ...
    if queue.enable_notifications_on_email_events and len(notifications_to_be_sent):
        ...
        ticket_cc_list = (
            TicketCC.objects.filter(ticket=ticket).all().values_list("email", flat=True)
        )
...
def object_from_message(message, queue, logger):
        ...
        if part.get_content_maintype() == "text" and name is None:
            if part.get_content_subtype() == "plain":
                ...
                if (
                    ...
                    and part.get_content_charset() == "utf-8"
                ):
                ...
                if ticket is None and getattr(
                    django_settings, "HELPDESK_FULL_FIRST_MESSAGE_FROM_EMAIL", False
                ):
        ...
        files.append(
            SimpleUploadedFile(
                _("original_message.eml").replace(
                    ".eml", timezone.localtime().strftime("_%d-%m-%Y_%H:%M") + ".eml"
                ),
    ...
    return create_object_from_email_message(
        message, ticket, payload, files, logger=logger
    )
