...
from modules.subscriptions.models import Packages, Sponsors


def get_packages(story, user=None):
    story = int(story)
    packages = (
        Packages.objects.filter(story=story)
        # .filter(story__author=user)
        .select_related("story").annotate(counts=Count("sponsor"))
    )
    print(packages)
    return packages
